...
public class SimpleExample {
	 ...
	 * The example expects the following parameters:
	 Maximum number of messages to read (so we don’t loop forever) 读取的最大消息个数
	 Topic to read from  读取的主题
	 Partition to read from 读取的分区
	 One broker to use for Metadata lookup
	 Port the brokers listen on broker的端口
	 * @param args
	 */
